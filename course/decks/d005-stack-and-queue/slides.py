...
SLIDES = [['受限的线性结构：栈与队列',
  ...
  '课程名'],
 ['明明可以随便存取，为什么要主动受限？',
  '<p>回收 D004 留问：生活里有三种“不自由”的结构——</p><ul><li>食堂的餐盘叠成一摞：只能从顶上放、从顶上取；</li><li>挤满人的电梯：先到的人后走；</li><li>编辑器的“撤销”：最后做的事，最先撤回。</li></ul><p>注意：它们的“不自由”是<strong>客观限制</strong>——餐盘压在下面，物理上就是够不着；先进电梯的人站在里面，只能后出。</p><div '
  'class="quote">而内存里的数据明明可以随便存取。<br>把客观限制变成主动选择的纪律，图什么？</div><details '
  'class="teacher-note"><summary>教学注记</summary><p>开场前先用 3–5 分钟回顾 D004 '
  '作业的共性错误（助教批改 + AI 预检汇总）：断链顺序（先接后继、再改前驱）与 free '
  '遗漏是高危点，各举一个典型错例即可——正好衔接本讲“限制与不变量”的主题。</p></details>',
  '回到问题'],
 ['一个新思想：给线性表加限制',
 ...
 ['front == rear，是空还是满？',
  ...
  'class="quote">少用一个格子，还是多记一个数——<br>工程权衡，没有免费午餐。</div><p '
  'class="small">见教材 3.5 节：循环队列的顺序存储与判空判满。</p><details '
  'class="teacher-note"><summary>教学注记</summary><p>口径提醒（布置选择题 (3) '
  '前必讲）：本课与教材正文的口径是 front 指向队头元素、rear 指向队尾元素的下一个位置；但选择题 (3) '
  '用的是另一种常见口径——“f 为队头元素的前一位置，r 为队尾元素的位置”。两种口径下判空判满公式形式相同，但“指针指向什么”不同。训练要点：读题先确认口径，再套公式，不要背结论。</p></details>',
  '循环队列'],
 ['手工推演：N = 5 的循环队列',
 ...
 ['约束不是损失，是设计',
  ...
  'class="teacher-note"><summary>教学注记</summary><p>“接口越窄、不变量越容易维护”对应软件工程的信息隐藏原理：模块暴露得越少，外部能破坏的东西就越少。可预告一句：以后看到的好的模块设计，都在做“限制”这门功课。</p><p>限制与自由的辩证法：D004 '
  '留问页已在课堂上充分展开（约束与自由、自我约束实例），此处回收一句即可——“上次说人生需要自我施加的约束，今天看到数据结构也一样：约束不是损失，是设计”——不要重新展开，避免对同一批学生重复。</p></details>',
  '本质收口'],
 ['把三个情境放在一起看',
 ...
 ['课后：教材导航与本周作业',
  ...
  '节（数制转换、表达式求值、舞伴问题），任选一个案例画出它的栈/队列轨迹。</p><details '
  'class="teacher-note"><summary>教学注记</summary><p>出门条（落地课程设计方案 §九 '
  '学生侧观测回路，3 分钟，不计分）：下课前请学生写两行——①今天收口的本质关系是什么；②一个还没懂的问题。快速翻一遍，共性问题下次课开场回应。</p></details>',
  '课后衔接'],
