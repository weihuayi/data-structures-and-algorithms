...
SLIDES = [['组织数据，让计算更有效',
 ...
 ['数据结构与算法',
  ...
  '<p class="small">算法组织操作；操作也可由算法实现。代码返回排序后的位置，取得原编号还需对应关系。</p>',
  '从专业术语回到课程主线'],
 ['算法的五个特性',
 ...
 ['枚举代价：由配对条件数出来',
  ...
  '<p>按每次配对检查为常数代价计，枚举最坏时间复杂度为 <strong>O(n²)</strong>。</p>',
  '规模与代价'],
 ['扫描很快，排序也要花时间',
 ...
 ['在几种表达之间来回检查',
  ...
  '情境：输出是否回答了原来的问题？</li><li>数据变化 → 方法：原来的条件和表示还成立吗？</li></ul>',
  '学习方法'],
 ['我怎样知道自己真的学会了？',
  ...
  '学习方法'],
 ['课后：完成三个对应任务',
  '<ol><li><strong>排序映射：</strong>写出位置、原编号、价格的对应；辨认数据元素、数据项与改变的关系。</li>\n'
  '<li><strong>目标 37：</strong>记录扫描轨迹，解释一次排除并写出不等式，说明为什么会终止。</li>\n'
  '<li><strong>程序核对：</strong>说明输入是否已排序、输出位置的含义，以及未计入的排序与存储成本。</li></ol>\n'
  '<p class="small">输出全部配对为可选挑战。具体练习要求以课程发布内容为准。</p>',
  '课后练习'],
 ['回到教材第一章',
  ...
  '<p class="small">阅读第一章对应内容与小结，再回看讲义中的案例说明。</p>',
  '教材接口 · 预习与复习']]
